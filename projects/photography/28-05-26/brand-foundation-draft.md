# Hadi Photography London — Brand Foundation Draft

**Drafted by:** Photography Strategist (Jodie)
**Date:** 28 May 2026
**Framework:** Maddie Mae / Adventure Instead "Thrive" — `Brand - Creating Your Irresistible Brand.md`
**Status:** PHASE 1 COMPLETE. All 11 sections LOCKED between 28 May and 4 June 2026 (3 days ahead of 7 June target).

**Final lock summary:**
- Section 1 Vision — LOCKED 28 May (Draft 1, people-first + legacy)
- Section 2 Mission — LOCKED 28 May (Draft 3b, craft-led, "every shoot I take on")
- Section 3 Values (5) — LOCKED 28 May (modified Set C: people first / conversation is the work / direction not documentation / relationship outlasts / earned calm)
- Section 4 Specialty — LOCKED 1 June (Maddie Mae Template #1, 3 differentiators: 20yr photojournalism / eye for anticipation + composition / 10yr working London)
- Section 5 Primary ICA — LOCKED 1 June ("Sarah and James" — London-led, city as third character)
- Section 6 Secondary ICA — LOCKED 2 June ("Sophie and Marcus" — Returned-to-London couples)
- Section 6 Tertiary ICA — LOCKED 2 June ("Emma and Ryan" — Destination couples, no London history)
- Section 7 UVP — LOCKED 2 June ("Your love has a city...")
- Section 8 Brand Voice — LOCKED 4 June (Refined Dreamer, no mixing)
- Section 9 Brand Story — LOCKED 4 June (full StoryBrand: Character + Problem / Guide / Plan / CTA / Avoided Failure / Achieved Success)
- Section 10 Reconciliation — LOCKED 4 June (Presence Method retired publicly, three-layer model preserved, old tagline retired, nervousness framing retired internally)
- Section 11 Validation — LOCKED 4 June (kill conditions set, Phase 2 handoff ready)

**Phase 2 starts 5 June 2026:** Website Rebuild (homepage rewrite first, per the photography rebuild action plan at projects/photography/28-05-26/photography-rebuild-action-plan.md).

**Canonical reference rule:** Every piece of customer-facing copy (meta descriptions, inquiry replies, blog posts, captions, pricing page, contact form, contracts, signatures) traces back to THIS document. Cross-check locked elements before clearing copy. Strict banned-word audit on every draft.

A note on method: every section below is built from the story bank, your client reviews, the existing positioning work, and observable pipeline patterns. Where a claim leans on a specific source, the source is referenced (e.g. "story bank entry #7"). Where a claim is inferred from limited data, it is flagged.

**Method update 28 May 2026:** workbook-first methodology now in force. All future drafting strictly follows Maddie Mae's templates and structures. No off-piste invention.

---

## 1. Vision Statement

**My vision is to give every couple I photograph the calmest, most present version of the most important day of their life — and to leave behind a body of work from London that proves what is possible when a photographer treats people first and pictures second.**

*Source:* Story bank entries #2 (the phone call, "I wanted to photograph people at the start of something"), #19 ("Technical skill gets you hired. Emotional skill gets you remembered."), #14 (the night-before advice). The two threads that run under everything Hadi has ever said about this work: photographing people at the start of something good after a career photographing the opposite, and the body of London work itself as the legacy.

---

## 2. Mission Statement

**My mission is to lead each couple through a deliberate process — a pre-wedding conversation, a pre-wedding shoot, and a wedding day run by someone who has spent twenty years learning how to read a room — so that the people in front of the lens forget the camera is there and the photographs follow as a result.**

*Source:* The pre-wedding shoot as mechanism (entry #5), "Authentic emotion is created by conditions, not commands" (entry #19), the photojournalism transfer / decisive moment (entry #4).

---

## 3. Core Values

Pulled from recurring language in the story bank, the 17 reviewed Google reviews, and Hadi's own framing of the work.

**1. Presence over performance.**
The photographer's job is to manage the room so the couple stops performing for the camera and starts being themselves. Hadi sends a "stare at each other, a lot" brief the night before a shoot. Most photographers send a logistics email. (Source: entry #14.)

**2. Earned calm.**
Twenty years of decision-making in unforgiving conditions — photojournalism, then weddings — produces a particular kind of calm in the room. It is not a personality trait. It is a skill. It is what couples are actually paying for. (Source: entries #4, #8.)

**3. Direction, not documentation.**
Hadi guides people through the shoot. He gives clear instruction, mid-session feedback, posing direction, simple actions that let real reactions surface. The reviews use the words "guided," "taught," "walked me through," "directed" — 5 separate clients independently. (Source: entry #9 Pattern 3.)

**4. Honesty about what the work costs.**
Hadi will tell a couple their wedding photographer should not be picked by lowest price, that a staged shoot has no friction, that some photographers cancel 48 hours out. He says the quiet thing out loud. (Source: entries #7, #10, Josh and Catherine #15.)

**5. The couple is the subject. Always.**
Not the dress, not the venue, not the light, not the photographer's portfolio aspirations. The couple. Every other choice serves them. (Source: entry #14 "It is all about you," entry #19 "Your presence changes the room.")

---

## 4. Specialty (sharpened)

**Hadi's specialty: a small, deliberate London elopement and pre-wedding shoot for couples who are quietly nervous about being photographed and want a photographer who will lead them through it.**

Not "London elopement photographer." Too broad — every London elopement photographer says that. The sharper specialty narrows on three vectors at once:

- **Size:** small / elopement / micro wedding (under 30 guests)
- **Location:** London (the body of work, the local knowledge, the 100 five-star reviews are all here)
- **Type of couple:** couples who arrive nervous, camera-shy, or quietly self-conscious — and want a photographer who actively guides them rather than "documents" from a distance

This is supported by the strongest single signal in the brand: 11 of 17 reviewed Google reviews use the nervousness-to-ease language unprompted. That is not a positioning choice. It is a description of what the work already does.

The Maddie Mae framework recommends combining 2–3 categories. This combines **size + location + type of couple** — the three sharpest cuts for Hadi's actual book of work.

---

## 5. The 4 Lists Exercise

### List 1 — MOST fulfilling sessions

Pulled from explicit story bank praise + reviews:

- **The 10th anniversary couple** (#6) — a long-married couple reconnecting through being photographed. "Falling in love again." Emotionally heavy, transformational session.
- **Josh and Catherine micro wedding** (#15) — last-minute rescue, small wedding, became close friends, twins later, ongoing relationship.
- **Alex and Juliet pre-wedding** (#1) — couple discovered things about each other through the shoot itself.
- **The camera-shy husband** (#11) — bride warned Hadi in advance, husband transformed mid-session, asked for more hours.
- **The proposal rescue from the US** (#9) — extreme pressure, 48 hours, stranger from another country, became one of the happiest clients.

**Common features:** small group (2-4 people), London, prior nervousness or vulnerability disclosed, Hadi-led conversation before the session, transformation during the session, emotional message after.

### List 2 — MOST valued clients

Built from the 17 reviewed Google reviews and the story bank:

- The skeptical mother who came in cynical and changed her mind by the end (review #6)
- The US editor at a major publication who hired Hadi sight unseen (review #3)
- International couples who took faith on a stranger from another country (reviews #8, #12, #6)
- Repeat clients (review #6, #8 — returned for maternity, intend to return for family)
- The thank-you booking — paid for a second session they would never attend (entry #12)
- Mozhgan — repeat client, three bookings, cash-on-the-day, low-friction (session log 22 May)

**Common features:**
- Educated, articulate, often professional (editor, doctor, lawyer, Google employee, business owner)
- Many international (US, EU)
- Many Persian / Iranian heritage (Mozhgan, the recent agency-side leads who also book photography signal a network)
- Pay attention to how they are treated, not just how the photos look
- Refer freely once the trust is established
- Skew slightly older than typical wedding market — 30s and 40s rather than mid-20s

### List 3 — LEAST fulfilling sessions

Hadi has not catalogued these explicitly in the story bank, but the negative space is visible:

- **Big traditional weddings dominated by family politics.** Hadi's pre-wedding mechanism cannot operate when 60 guests, parents, and a strict schedule are the centre of gravity. Implied by entry #8 ("a staged shoot has no friction. Weddings do") and entry #10 ("the day moves like a blur").
- **Lowest-price shoppers.** Couples who assume all photographers deliver roughly the same thing and pick on price. Directly named in entry #8.
- **Couples who never have the pre-wedding conversation.** The mechanism fails. Entry #5 was created in direct response to this pattern of disappointment.
- **Staged / styled shoots with models.** Entry #8 — Hadi names them explicitly as misleading and not what he wants to be photographing.

*Flagged: these are inferred from the existing material. Hadi to confirm or add specifics.*

### List 4 — LEAST valued clients

Inferred from the same material plus the pipeline pattern:

- Price-shoppers who treat photography as a commodity ("how many hours do you include?" — entry #8)
- Couples who under-communicate expectations and then arrive disappointed (the original pre-wedding shoot problem, entry #5)
- Clients with rigid pre-conceived ideas who do not allow Hadi to direct them
- Clients who do not value the photographer as a person — only as a service provider

*Flagged: inferred. Confirm with Hadi.*

### Themes that emerge

1. **The session size that fits Hadi's mechanism is small.** Two people, four people, ten people. Not 80. The pre-shoot conversation, the night-before advice, the in-session direction — all of it only works at small scale.
2. **The right client is articulate and observant.** They notice how they are treated. They write reviews that quote what Hadi said to them. They become friends.
3. **Trust is the deepest currency.** Not portfolio. Not price. International clients hire him sight unseen. Repeat clients book without negotiation. Persian/Iranian community refers via personal endorsement.
4. **Hadi's instinct is to lead, not document.** The reviews that use "guided," "taught," "directed," "walked me through" outnumber every other category. This is the actual product.
5. **The work compounds on relationships.** Friendships from shoots, ongoing contact, repeat bookings, twins, planned trips to the US. This is a relationship business disguised as a photography business.

---

## 6. Primary Ideal Client Avatar — LOCKED 1 June 2026

### PRIMARY ICA — "Sarah and James" (London-led: city as third character)

**Description — Who**

- **Names & pronouns:** Sarah (she/her) and James (he/him). Representative names. Actual primary couples vary across British, European, American, Canadian, Middle Eastern, Australian backgrounds.
- **Ages:** 28-36 (within the broader 25-42 range Hadi sees)
- **Occupations:** Two demanding professional careers. Most common patterns: medical (consultant, surgeon, doctor); legal (lawyer, barrister); aviation (pilot); tech (CTO, head of engineering, founder); leadership (CEO, founder, senior business leader); adjacent fields (architect, designer, finance, consulting). Almost always two demanding careers, not one professional + one supporting partner.
- **Location:** Either London-based (Hampstead, Notting Hill, Marylebone, South Kensington, Islington, Greenwich, Richmond, Chelsea, Wandsworth) OR international with London ties (US East Coast, California, Toronto, Vancouver, Paris, Berlin, Stockholm, Amsterdam, Dubai, Sydney).
- **Education:** Highly educated. Both partners typically have undergraduate degrees from reputable universities. Many have masters, MBAs, medical degrees, or professional qualifications.
- **Combined income:** £150,000-£400,000+
- **Relationship:** Together 2-10 years. Many have lived together 1-3+ years before engagement. The relationship has weathered career moves, geographic moves, big decisions.
- **Cultural background:** Mixed. Often one or both partners have diaspora background. Unifier is cosmopolitan outlook, comfort across cultures, not bound to a single traditional wedding script.

**Behaviour & personality — What**

- **Hobbies & interests:** Walk the city for fun (Hampstead Heath, Greenwich, Regent's Canal, Hyde Park, Richmond Park). Favourite restaurants in Soho, Marylebone, Mayfair, Chelsea, Borough Market. Long-haul travel multiple times a year plus European weekends. One usually reads serious nonfiction; both follow design, food, travel content on Instagram. Cultural events. Socialise in small groups, not large parties.
- **Spending pattern:** Travel non-negotiable. Hotels rated by Mr & Mrs Smith or The Standard. Will pay for the chef's table. Considered home design, each piece chosen. Quality clothing without logos. They will pay for craft. They will not pay for marketing.
- **Sources of information:** Google for research. Instagram for inspiration, filtered aggressively. Direct referrals from friends (primary source for service decisions like photography). The Cut, Monocle, Cereal, Kinfolk, FT Weekend, Times Saturday supplement. NOT Pinterest. NOT wedding blogs.
- **Personal style:** Restrained, considered, neutral palette, no logos. Their home looks like a quiet design magazine spread.

**Motivators — Why**

**Goals, desires, values for their wedding/elopement:**
- Want the day to feel like THEM, not a template or someone else's wedding
- Want photographs that hold what actually happened: real expression, not arranged smiles
- **Want London to BE part of the day, not just the location.** The city is something they love. They want it in the photographs as a third character.
- Want to actually be present on the day, not running it
- Want photographs they will look at in ten years and still feel something
- Want a photographer whose tone matches theirs: considered, not flashy

**What they want family and friends thinking:**
- "That felt like watching the two of you."
- "I've never seen them like that."
- "London looked like London in those photos, not a stock-photo London."

**Core values shown in their life choices:**
- Quality over quantity
- Craft over flash
- Restraint over performance
- Being present over documenting
- Travel as discovery, not status
- London as home, even when they live elsewhere

**Challenges / pain points:**
- Time. Both partners overworked. Wedding planning cannot be a full-time project.
- Distance if international: planning across timezones and continents
- The wedding industry does not speak their language: most wedding content is built for someone louder, more performative, more conventional
- Family expectations, particularly if one or both have cultural pressure about what a wedding "should" be
- Choosing a photographer they have never met (especially if international): the risk feels material
- Choosing locations in London they cannot easily scout themselves
- They have seen beautiful wedding photographs and then realised the same photographer did 100 weddings that all look the same. The fear of looking like everyone else.

**Fears, internal struggles:**
- "I am going to look stiff in the photos."
- "We will spend a lot of money and end up with photos that look like everyone else's."
- "We do not have the expertise to tell good photographers apart."
- "If we book the wrong person, the day itself becomes worse, with no recovery."
- For international couples: "We are trusting someone we have never met, in a country we may not live in, with the most important day of our lives."
- "We chose London because we love it. What if the photographer does not understand WHY we chose London?"
- "The wedding industry assumes we want something we do not want. How do we find someone who actually listens?"

**Possible objections:**
- "Why is this more expensive than the photographer on Instagram whose work looks similar?"
- "We were thinking of just 2-3 hours. Is that enough?"
- "Do we really need a pre-wedding shoot? Our schedules are brutal."
- "We do not want this to feel cinematic or styled. Can you actually do that?"
- "Can you photograph London in a way that does not look like every other photographer's London?"
- For international: "How do we know you will show up? How do we know you understand what we want without meeting?"
- "Can you handle the planning so we do not have to manage it?"

### SECONDARY ICA — "Sophie and Marcus" (Returned to London) — LOCKED 2 June 2026

**Description — Who**

- **Names & pronouns:** Sophie (she/her) and Marcus (he/him). Representative. Actual returned couples come from US (East Coast, California), Canada, Western Europe, Middle East, Australia.
- **Ages:** 31-40. Slightly older than Primary because the London chapter was in their 20s and they're returning years later.
- **Occupations:** Professional — medical (consultant, surgeon), legal (international law, M&A), aviation, tech (CTO, founder), finance (banking, asset management, private equity), leadership (CEO, founder), or creative-professional. London chapter often during university (UCL, LSE, Imperial, KCL, SOAS) OR 2-5 years early-career work.
- **Where they live now:** US East Coast, California, Toronto, Vancouver, Paris, Berlin, Stockholm, Amsterdam, Singapore, Sydney, Dubai.
- **Where they USED to live in London:** Zones 1-3 (Bloomsbury, South Kensington, Chelsea, Marylebone, Notting Hill, Hampstead, Islington, Shoreditch).
- **Education:** Highly educated. Often masters, MBA, JD, MD obtained in London or later.
- **Combined income:** £180,000-£500,000+ (higher than Primary — further along in careers).
- **Relationship length:** Together 7-15 years. Relationship STARTED in London or was substantially shaped by London years.
- **Cultural & family background:** Often international heritage. London years were when they figured out who they wanted to be.

**Behaviour & personality — What**

- **Hobbies:** Travel often. Have favourite spots from their London years they still visit. Read serious publications. Often have a "London friend group" they see on visits. Cultural events when in London (Tate, Royal Opera House, Sadler's Wells, Barbican).
- **Spending pattern:** Travel non-negotiable. Business class long-haul. Boutique hotels with character. Will pay for "the London they remember." Established taste, considered choices. Higher-spend than Primary.
- **Sources of information:** Google. Direct referrals INCLUDING old London friends. The Cut, Monocle, FT Weekend, Times Saturday. NOT Pinterest, NOT wedding blogs.
- **Personal style:** Settled, considered, established. Home reads as someone who has lived in multiple cities and brought favourites from each.

**Motivators — Why**

**Goals, desires, values:**
- The day is partly a HOMECOMING. London is part of the love origin.
- Want photographs that hold what London meant to them at the earlier chapter, plus what it means now
- Want a photographer who UNDERSTANDS why they chose London (many haven't articulated this fully)
- Want to revisit places that mattered (the pub where one first asked the other out, the bench, the cafe where they planned their futures)

**What they want family and friends thinking:**
- "Of course they went back to London. That's where it started."
- "Those photographs feel like THEM, not generic engagement photos."
- "London looks like London-the-place-they-loved, not London-the-stock-image."

**Core values:**
- Returns over conquests
- Substance over style
- Personal history over public spectacle
- The relationship outlasts the chapter

**Challenges / pain points:**
- Planning from another country, often another timezone
- The locations that mattered may have changed
- Limited time in London (4-7 days)
- The wedding industry has no template for "come back to your former city"

**Fears, internal struggles:**
- "Will the photographer understand this isn't just 'a London engagement shoot'?"
- "Will they make us look like tourists in our own former city?"
- "What if the spots that mattered don't photograph the way they felt?"
- "We're trusting someone we've never met, in a country we used to live in, with the day that maps the chapter that defined us."

**Possible objections:**
- "How do we know you'll understand WHY these specific locations matter?"
- "Will you take suggestions, or push us toward your favourite spots?"
- "Can we plan this entirely remotely?"
- "Pre-wedding shoot the day before — do we have time?"

**Live pipeline example:** Hedy Rafii + Omid Mohareri (enquiry 2 June 2026) fits this Secondary ICA almost exactly. Josh and Catherine (story bank entry #15) is the documented prior case.

### TERTIARY ICA — "Emma and Ryan" (Destination Couple, No London History) — LOCKED 2 June 2026

**Description — Who**

- **Names & pronouns:** Emma (she/her) and Ryan (he/him). Representative. Actual destination couples come from US, Canada, Western Europe, Middle East, Australia, Hong Kong, Singapore.
- **Ages:** 28-42.
- **Occupations:** Same professional bracket — medical, legal, aviation, tech, leadership, finance, creative-professional. Both partners with careers.
- **Where they live:** Anywhere except London. NEVER lived in the UK.
- **Education:** Highly educated. Often masters or professional qualifications obtained at home.
- **Combined income:** £150,000-£400,000+
- **Relationship length:** Together 3-10 years.
- **Cultural & family background:** Mixed. Unifier: cosmopolitan outlook + love of cities + deliberate love of London.

**Behaviour & personality — What**

- **Hobbies:** Travel as a way of life. London is one of several cities on a list. Have visited London 2-5 times before booking. Follow London-related content. City-curious, read travel content, follow architecture and food on Instagram.
- **Spending pattern:** High travel spend. Business class long-haul. Central London hotels (Bloomsbury, Marylebone, Covent Garden, Mayfair, Shoreditch). Will pay for craft and considered work. Travel itself is the indulgence.
- **Sources of information:** Google, Instagram for inspiration, Monocle, Cereal, Wallpaper, FT How to Spend It, NYT Travel. NOT Pinterest, NOT wedding blogs.
- **Personal style:** Considered, well-travelled, refined. Home shows influence of multiple cities.

**Motivators — Why**

**Goals, desires, values:**
- London as DELIBERATE destination — they chose it
- Want the session to feel European, layered, not standard American engagement photography
- Want the photographer to introduce them to the London the postcards miss
- Engagement session often part of a larger trip (1-2 weeks)
- Will want a few iconic shots for family + the less-obvious work for themselves

**What they want family and friends thinking:**
- "They actually went TO London for engagement photos. That's commitment."
- "These photographs look like Europe, not like a stock-photo London shoot."

**Core values:**
- Travel as discovery
- Cities as identity
- Choose unusual things deliberately

**Challenges / pain points:**
- They don't know London. Have visited but haven't lived it.
- Depend ENTIRELY on the photographer's knowledge of locations
- Tight schedule in London (4-7 days, much of which is touristing or work)
- The wedding industry assumes destination = big production. They're choosing the opposite.
- Trust at distance is the central challenge

**Fears, internal struggles:**
- "Will the photographer take us to interesting London or generic-tourist London?"
- "We've never met this person. We're flying eight hours for this."
- "We don't speak London. How do we communicate what we want?"
- "What if the weather is bad and the day gets ruined?"

**Possible objections:**
- "Why book you specifically rather than another London photographer?"
- "Can you show us examples of locations you'd take us to?"
- "What if it rains?"
- "We've seen photographers in [home country] who'd fly over for less. Why is this priced where it is?"

---

## 7. Unique Value Proposition — LOCKED 2 June 2026

**Locked UVP (Draft 6 — "London as witness"):**

> **"Your love has a city. Whether you found it in London, came back to London for it, or are bringing it to London for the first time, my work is to photograph the day so the city stands as a witness to the relationship, not a backdrop for it."**

**Why this works:**
- Speaks to all three locked ICAs in their own emotional language: Sarah and James ("the city you love"), Sophie and Marcus ("came back to London for it"), Emma and Ryan ("bringing it to London for the first time")
- "Your love has a city" is a sentence almost no one in the competitive set would write — too poetic for a feature-led photographer, too direct for a generic wedding photographer
- Directly addresses the deepest fear in the Primary ICA profile: *"What if the photographer doesn't understand WHY we chose London?"*
- No banned words. No em dashes.

**Preserved alternates** (Hadi flagged 2 June for potential revisit):

*Alternate 1 (Draft 4 — the "what you're really choosing" angle):*
> "You are not just choosing a photographer for your day in London. You are choosing a city you love over a city that is convenient, a day you cannot redo over photographs you might never look at again. I will treat both choices with the seriousness they ask for."

*Alternate 2 (Draft 5 — the "you will be seen" angle):*
> "Twenty years of photographing people on the most important days of their lives has taught me one thing: the photographs you will not flinch at in ten years are the ones where you were actually seen, not just looked at. That is what I am here to do for you in London."

**Note on the original strategist draft** (replaced 2 June): the strategist's earlier UVP led with "couples who quietly dread being photographed" — Hadi pushed back hard on deficit framing across multiple sections (2 June) and the locked Draft 6 has zero deficit framing.

---

## 8. Brand Voice — LOCKED 4 June 2026: REFINED DREAMER

**Locked voice: Refined Dreamer.**

Per Maddie Mae's framework, all customer-facing copy must commit to ONE voice — mixing creates inconsistency. Hadi locked Refined Dreamer after hearing the same Mission + UVP content written in all 4 voices side by side.

**Why Refined Dreamer:**
- Matches the register of the locked UVP ("Your love has a city")
- Matches the emotional weight of the decision the couples are making (spending thousands on a wedding day in a city far from family and friends)
- Provides emotional resonance that Straight No Chaser was too cold to deliver
- Reads as appropriate to the £1,400-£5,100 commission range

**Reference paragraph in the locked voice** (Mission + UVP combined):

> Your love has a city. Perhaps you found it in London, walking the same Bloomsbury streets year after year. Perhaps you came back to London for it, the city quietly holding the chapter that shaped you. Perhaps you are bringing it to London for the first time, choosing the city the way some couples choose each other: slowly, deliberately, with the sense that some places hold meaning long before you arrive in them. My work is to photograph the day so the city stands as a witness to the relationship. Twenty years of photojournalism have trained me to notice the half-second before the gesture, the breath before the laugh. The photographs hold what was actually there.

**Critical implementation rules:**

1. **No mixing with other voices.** Inquiry replies, captions, pricing pages, contact thank-you pages, contracts, signatures — all in Refined Dreamer from here forward.
2. **Banned-word vigilance is non-negotiable.** Refined Dreamer's risk is drifting into the exact cliché vocabulary the brand must avoid (intimate, moments, story, authentic, capture, memories, etc.). Every draft from here forward gets a strict Control-F check before clearing.
3. **The Hedy opener sent 2 June was Straight No Chaser**, not Refined Dreamer. It still works for that one message, but the inquiry workflow rebuild in Phase 3 will be in Refined Dreamer.
4. **What "Refined Dreamer" means in practice for this brand:** poetic but considered, sensory but restrained, emotional but not precious. Slightly elevated vocabulary without slipping into wedding-photographer cliché. Pace can breathe. Sentences can extend when emotion calls for it. Specific over abstract — name the Bloomsbury street, not "a quiet street in London."

**Discarded options and why:**

- **Straight No Chaser** — closest to Hadi's natural conversational voice, but reads too cold against the emotional weight of the UVP and the locked ICAs' fears (looking stiff, not being seen, spending money on photos that look like everyone else's). Strong on substance, weaker on emotional warmth.
- **Client BFF** — too casual for the £1,400-£5,100 commission level. The ICAs are looking for craft and seriousness, not friendliness.
- **Hype Human** — wrong category, wrong client base.

**Original strategist recommendation** (replaced 4 June): the strategist had recommended Straight No Chaser with a borrowed Refined Dreamer element. Hadi's decision after hearing all 4 voices on the same content was the opposite call — Refined Dreamer as the foundation, with the discipline of Straight No Chaser's short sentences available when needed.

---

## 9. Brand Story (StoryBrand framework)

**Character:**
A couple in their thirties planning a small London wedding or pre-wedding shoot. Educated, considered, quietly nervous about being photographed. They want the day to feel like them.

**Problem — external:**
They cannot tell which London elopement photographer to trust. The portfolios look similar. The pricing is opaque. The wedding industry pushes them toward a louder, more performative day than they want.

**Problem — internal:**
They are afraid they will look stiff in the photos. They are afraid they will spend the entire day self-conscious about being watched, and end up with images that prove it. One of them is more reluctant than the other (entry #11 pattern) and is privately dreading the shoot.

**Problem — philosophical:**
The day that should be theirs has been colonised by an industry that treats couples as decoration for the photographer's portfolio. The photographs should be of the couple. They are increasingly of the venue, the dress, the light, and the photographer's aesthetic choices.

**Guide:**
Hadi. Empathy: he has spent twenty years watching nervous people on the most important days of their lives, and he has noticed the same thing every time — they relax when someone runs the room for them. Authority: photojournalism in difficult environments, 100 five-star Google reviews, 11 of 17 reviewed clients independently using the same nervousness-to-ease language, repeat international clients who hired him sight unseen.

**Plan:**
1. **A real conversation before anything is booked.** Hadi learns what the couple is actually nervous about. (Source: the camera-shy husband, entry #11.)
2. **A pre-wedding shoot as the mechanism, not the add-on.** A low-stakes session where the couple learns how Hadi works and Hadi learns how to direct them. (Source: entry #5.)
3. **A wedding day run by someone who reads rooms for a living.** Clear direction, calm presence, the decisive moment training. (Source: entries #4, #19.)
4. **A night-before brief that is not logistics.** "Stare at each other. A lot." (Source: entry #14.)

**CTA:**
Send an enquiry. Have the conversation. See if it fits.

**Avoided failure:**
- Stiff, self-conscious photos that do not look like them
- A day spent performing for a camera instead of being in the moment
- Being treated as a client number rather than two specific people
- The disappointment of paying premium prices for generic London elopement images

**Achieved success:**
- A photographer they actually like
- A pre-wedding shoot where they noticed something new about each other (entry #1, "It felt like falling in love again")
- A wedding day where the camera stopped existing for them
- Photographs they will not flinch at in ten years
- A relationship, not a transaction (Josh and Catherine, the thank-you booking, the 10th anniversary couple)

---

## 10. Reconciliation with Existing Work

**The Presence Method → "The Guide's Plan" in StoryBrand.**
The Presence Method is the four-step plan above (conversation → pre-wedding shoot → wedding day → night-before brief). It does not disappear under the new framework. It becomes the named methodology that lives under the UVP. In copy it can be referenced by name once Hadi confirms he wants to keep the proprietary-mechanism language.

**Elevation → Guidance → Relief (three-layer model) → macro copy sequence.**
This is the order in which copy is laid out on the homepage and in inquiry replies. Elevation = lead with the transformation (entry #1, entry #6 — falling in love again). Guidance = the Presence Method as the plan. Relief = the 100 five-star reviews, the repeat international clients, the named transformations. The three-layer model is structurally sound. It is not replaced — it is sequenced into the StoryBrand framework above.

**Nervousness-to-ease transformation.**
This is now the central brand promise. It is in the UVP. It is in the brand story (internal problem + achieved success). It is the strongest single signal in the reviews. It stays. It gets reinforced.

**100 five-star Google reviews.**
This is the strongest social proof in the brand. It belongs above the fold on the homepage and in every inquiry reply.

**What stays verbatim:**
- "Your elopement day should be the best day of your life. I make sure it is." — this line still works as a tagline. It does not conflict with the new UVP. It can sit underneath. Hadi to decide.
- "Stare at each other. A lot." — too good to lose.
- "You're paying for 20 years of experience compressed into one day." — pricing-page-anchor language.
- "I don't just see people. I see anticipation." — about-page or homepage emotional hook.

**What needs revising:**
- Any current copy that uses "intimate," "captured," "story," "moments," "documentary," "authentic," "luxury," "high-end," "cinematic" (style — not London). All on the combined banned list. All have to go.
- Any current copy that leads with the venue, the dress, or "London" instead of the couple. The couple is the subject.
- Pricing-page language that frames packages by hours. Frame by experience type ("The Vow", "The Bloom", "The Glow", "The Odyssey") with a clear value-anchor explanation per Hadi's own argument in entry #8.

---

## 11. What Hadi Needs to Decide / Validate

1. **Sharpen the specialty or keep it broad?** The draft narrows hard: small London elopements and pre-wedding shoots for nervous couples. This deliberately deprioritises larger weddings (60+ guests), corporate portraiture, and the broader couples market. Is Hadi willing to position-out of that work, knowing the goal is to dominate the narrower segment? If not, the specialty needs widening — but the UVP and ICA will weaken with it.

2. **Lock Straight No Chaser as the brand voice?** The existing site reads as Refined Dreamer. Switching to Straight No Chaser is a significant copy rewrite across homepage, about, pricing, contact, inquiry workflow, and email templates. Hadi to confirm before the copywriter is briefed.

3. **Is the tertiary ICA (Persian / Iranian London couples) a distinct avatar or a referral channel?** Both readings are defensible. The answer changes whether we build separate copy for that audience or treat them as a network feeding the primary ICA.

4. **Keep "The Presence Method" as a named mechanism or absorb it into the StoryBrand plan without a brand name?** Named mechanisms produce stronger conversion when they are credibly owned. Named mechanisms also create more work to maintain. Hadi's call.

5. **Pre-wedding shoot — bundle in or sell separate?** Current pricing has it included from The Glow upward, standalone at £680. The Maddie Mae framework would push for "bundle into every commission" because the mechanism only works when every couple does one. Hadi to decide whether the bundling logic gets tightened.

6. **Banned word audit on the live site.** Hadi to greenlight a full Control-F sweep of hadiphotographylondon.com against the combined banned list (Maddie Mae's 25-word list + Hadi's pre-existing rules). Findings get routed to photography/copywriter for rewrite. This is the first concrete execution step after the brand foundation is locked.

7. **The kill condition.** If the rewritten homepage + inquiry workflow under this new positioning does not produce a measurable lift in either (a) inquiry-form conversion rate, or (b) inquiry-to-booking close rate within 90 days of going live, the positioning gets revisited. Hadi to set the specific numeric threshold.

---

## Drafting notes for Hadi

Three things I want to surface before you start marking this up:

**Tension 1 — the specialty is narrower than the current pricing structure assumes.**
The Presence Method packages include The Odyssey at £5,100 for 10-hour / destination / multi-day work. That is a full wedding, not a small London elopement. If we lock the narrower specialty, The Odyssey is the odd one out in the package range. It can stay as the "if you ask" tier without being marketed, or it can be revisited.

**Tension 2 — the nervousness-to-ease story is a fragile differentiator if it is the only one.**
Other photographers will copy the language once it works. The defensible moats underneath it are (a) the 100 reviews, (b) the photojournalism background, (c) the pre-wedding shoot as a baked-in mechanism, and (d) Hadi's actual presence in the room. The brand needs to reinforce all four, not just the transformation phrase. The UVP above tries to carry all four; the copy that comes out of this draft has to do the same.

**Tension 3 — the secondary ICA (international couples) needs distinct inquiry workflow copy.**
A US couple hiring sight unseen needs a different first email than a London couple who can have coffee with Hadi. The current single inquiry workflow does not address this. The Maddie Mae elopement email templates explicitly handle this distinction. When the email workflow rebuild starts, this gets built in from the start — not added later.

End of draft.
